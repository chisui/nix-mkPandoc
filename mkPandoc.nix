...
let
  ...
  templates = import ./template { inherit pkgs; };
  ...
  customTexlive = optional (to == "latex" || hasSuffix ".pdf" name) (
    texlive.combine (
      ...
      // (if template != null
      then template
      else templates.latex.default).texlivePackages or { }
    )
  );
  ...
  pandocArgs =
    ...
    in
    mkArgs normalArg (filterAttrs isRelevantArg args)
